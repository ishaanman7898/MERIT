...
impl Layer0Engine {
    ...
    pub fn process(&self, tx: &Transaction) -> Layer0Decision {
        ...
        if let Some((similarity, attack_id)) = lsh_result {
            if similarity >= self.lsh_similarity_hard_reject {
                ...
                return Layer0Decision {
                    pass: false,
                    reason: Layer0Reason::SimilarToAttack(attack_id),
                    velocity_score,
...
mod tests {
    ...
    fn test_lsh_similar_detection() {
        ...
        // Add a known attack
        let attack_features = vec![0.9, 500.0, 1000000.0, 68.0, 1.0, 100.0, 0.0, 3.0];
        lsh.add_attack("flash_loan_01".to_string(), attack_features.clone());

        // Slightly mutated version should still match
        let mutated = vec![0.88, 480.0, 950000.0, 72.0, 1.0, 95.0, 0.0, 3.0];
        let result = lsh.find_most_similar(&mutated);
        assert!(result.is_some(), "Should find similar attack");

        let (similarity, id) = result.unwrap();
        assert!(similarity > 0.7, "Similar attack should have high similarity");
        assert_eq!(id, "flash_loan_01");
    }
